...
NETWORK_ID = 137
POLYGON_DERIVATION_PATH = "m/44'/60'/0'/0'/{0}".format(NETWORK_ID)
...
def send_polygon_transaction(tx: Dict, passphrase: str) -> Dict:
    if not passphrase:
        raise ValueError("Passphrase argument is required!")

    gas_metrics = fetch_gas_oracle()

    account = generate_polygon_wallet(passphrase=passphrase)
